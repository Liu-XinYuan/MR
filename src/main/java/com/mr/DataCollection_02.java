...
public class DataCollection_02 {
	private static final Logger log = Logger.getLogger(DataCollection_02.class
			.getName());
	...
	public static void main(String[] args) throws IOException,
		...
		Job job = new Job(conf, "dc_real_02");
		job.setJarByClass(DataCollection_02.class);
		...
		String[] aces = { "base", "loc", "pc", "apklist", "apkhis", "apkuse",
				"apkdef", "apkf", "gps", "blue", "url", "phonelst", "schedule",
				"sdcard", "earphone", "charging", "cable", "phonerec",
				"photolst", "messagecon" };
		Set<String> mosSet = new HashSet<String>();
		...
		// Text.class, Text.class);
		// MultipleOutputs.addNamedOutput(job, "apklist",
		...
		// Text.class, Text.class);
		// MultipleOutputs.addNamedOutput(job, "phonelst",
		// TextOutputFormat.class,
		// Text.class, Text.class);
		// MultipleOutputs.addNamedOutput(job, "schedule",
		// TextOutputFormat.class,
		// Text.class, Text.class);
		// MultipleOutputs.addNamedOutput(job, "sdcard", TextOutputFormat.class,
		// Text.class, Text.class);
		// MultipleOutputs.addNamedOutput(job, "earphone",
		// TextOutputFormat.class,
		// Text.class, Text.class);
		// MultipleOutputs.addNamedOutput(job, "charging",
		// TextOutputFormat.class,
		// Text.class, Text.class);
		// MultipleOutputs.addNamedOutput(job, "cable", TextOutputFormat.class,
		// Text.class, Text.class);
		// MultipleOutputs.addNamedOutput(job, "phonerec",
		// TextOutputFormat.class,
		// Text.class, Text.class);
		// MultipleOutputs.addNamedOutput(job, "photolst",
		// TextOutputFormat.class,
		// Text.class, Text.class);
		 ...
		 */
		List<Path> gzList = getAllPath(fs, new Path(otherArgs[0]));
		...
		for (Path path : gzList) {
			...
			String dt = getDate(path.toString());
			if (dt != null) {
				mosSet.add(dt);
			} else {
				mosSet.add("0000-00-00");
			}
		}

		/** 注册 MultipleOutputs 多文件输出 */
		for (String ac : aces) {
			for (String dt : mosSet) {
				String fileName = (dt + ac).replace("_", "").replace("-", "");
				MultipleOutputs.addNamedOutput(job, fileName,
						TextOutputFormat.class, Text.class, Text.class);
			}
		}
	 ...
	 * @return
	 */
	private static String getDateFromPath(
	...
	public static class CollectionMapper extends
		...
		public void map(Object key, Text value, Context context)
			...
			try {
				...
				if (ac.equals("head")) {
					String imsi = jsonObject.getString("im").trim();
					String imei = jsonObject.getString("is").trim();
					String uuid = jsonObject.getString("uuid").trim();

					flag = imsi + "_" + imei + "_" + uuid + " ";

					// 最后添加一个空格，防止数据split 的个数为不合法
					// flag = (imsi.length() == 0 ? "null" : imsi) + "_"
					// + (imei.length() == 0 ? "null" : imei) + "_"
					// + (uuid.length() == 0 ? "null" : uuid);
				}
				if (flag == null || flag.split("_").length != 3) {
					// 还没标记
					return;
				}
				valueString = flag + "\t" + line;

				context.write(new Text(ac + ":" + dt), new Text(valueString));
	...
	public static class CollectionCombiner extends
		...
		protected void reduce(Text key, Iterable<Text> values, Context context)
			...
			String[] ac_dt = key.toString().split(":");
			if (ac_dt == null || ac_dt.length != 2) {
				log.error("combiner ac_dt 解析错误：key = " + key.toString());
				return;
			}
			String ac = ac_dt[0].trim();
			String dt = ac_dt[1].trim();
			...
			for (Text value : values) {
				...
				try {
					...
					if (ac.equals("base")) {
						...
						// mos.write("phonelst", new Text(result), new
						// Text(""));
						// context.write(new Text("phonelst"), new
						// Text(result));
					} else if (ac.equals("schedule")) {
						result = schedule_CL(baseInfo, jsonObject);
						// mos.write("schedule", new Text(result), new
						// Text(""));
						// context.write(new Text("schedule"), new
						// Text(result));
					} else if (ac.equals("sdcard")) {
						result = sdcard_CL(baseInfo, jsonObject);
						// mos.write("sdcard", new Text(result), new Text(""));
						// context.write(new Text("sdcard"), new Text(result));
					} else if (ac.equals("earphone")) {
						result = earphone_CL(baseInfo, jsonObject);
						// mos.write("earphone", new Text(result), new
						// Text(""));
						// context.write(new Text("earphone"), new
						// Text(result));
					} else if (ac.equals("charging")) {
						result = charging_CL(baseInfo, jsonObject);
						// mos.write("charging", new Text(result), new
						// Text(""));
						// context.write(new Text("charging"), new
						// Text(result));
					} else if (ac.equals("cable")) {
						result = cable_CL(baseInfo, jsonObject);
						// mos.write("cable", new Text(result), new Text(""));
						// context.write(new Text("cable"), new Text(result));
					} else if (ac.equals("phonerec")) {
						result = phonerec_CL(baseInfo, jsonObject);
						// mos.write("phonerec", new Text(result), new
						// Text(""));
						// context.write(new Text("phonerec"), new
						// Text(result));
					} else if (ac.equals("photo_lst")) {
						result = photo_lst_CL(baseInfo, jsonObject);
						// mos.write("photolst", new Text(result), new
						// Text(""));
						// context.write(new Text("photolst"), new
						// Text(result));
					} else if (ac.equals("message_con")) {
						result = message_con_CL(baseInfo, jsonObject);
						// mos.write("messagecon", new Text(result), new
						// Text(""));
						// context.write(new Text("messagecon"), new
						// Text(result));
					} else {
					...
					String keyString = (dt + ac).replace("_", "").replace("-",
							"");
					context.write(new Text(keyString), new Text(result));
				} catch (Exception e) {
					// TODO Auto-generated catch block
					log.error(e);
					continue;
		...
		private String message_con_CL(String[] baseInfo, JSONObject jsonObject)
			...
			try {
				...
				for (int i = 0; i < mes_lst.length(); i++) {
					...
					String msg = mes_lst.getString(i).trim();
					...
					if (msg.startsWith("[") && msg.endsWith("]")) {
						...
						for (int k = 0; k < msgArray.length(); k++) {
							arrayString += msgArray.getJSONObject(i)
									.getString("mes").replace("\r\n", "")
									.replace("\n", "").replace(",", "，")
									+ ",";
						}
					} else {
		...
		private String[] baseInfoCL(String line) {
			...
			try {
				...
				// 去掉map阶段添加的空格
				baseInfo[2] = baseInfo[2].substring(0, baseInfo.length - 1);
			} catch (Exception e) {
	...
	public static class CollectionReducer extends
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			for (Text value : values) {
				mos.write(key.toString(), value, new Text(""));
			}
		}
